import type { AIInsight, AppState, Project, ThoughtItem } from "../domain/types";
import { readiness } from "../domain/readiness";
import { now } from "../domain/utils";

const allowedThoughtKeys = ["title", "content", "type", "status", "universeId", "why", "outcome", "nextAction"] as const;
const allowedProjectKeys = ["name", "intent", "status", "readiness", "nextAction", "universeId"] as const;
...
export function applyAiInsightPatch(state: AppState, insight: AIInsight): AppState {
  ...
  for (const operation of insight.patch.operations as unknown[]) {
    ...
    if (operation.type === "updateThought" && targetType === "thought" && operation.thoughtId === targetId) {
      const safePatch = pickAllowed<ThoughtItem, (typeof allowedThoughtKeys)[number]>(operation.patch, allowedThoughtKeys);
    ...
    if (operation.type === "updateProject" && targetType === "project" && operation.projectId === targetId) {
      const safePatch = pickAllowed<Project, (typeof allowedProjectKeys)[number]>(operation.patch, allowedProjectKeys);
